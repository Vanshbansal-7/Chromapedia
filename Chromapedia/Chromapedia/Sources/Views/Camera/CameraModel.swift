// Chromapedia
// Views/Camera/CameraModel.swift — v3 (fixed hex accuracy)
...
final class CameraModel: NSObject, ObservableObject {
    ...
    private nonisolated func configureSession() {
        ...
        // Use 32BGRA — standard iOS camera format (byte order: B G R A)
    ...
    /// Safely grab the latest pixel buffer snapshot
    ...
    // Sample a 40×40 centre patch for stable, accurate color detection
    private func sampleCenterRegion() {
        ...
        // Only push update if color changed meaningfully
...
extension CameraModel: AVCaptureVideoDataOutputSampleBufferDelegate {
    nonisolated func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        // Thread-safe buffer update — retain the pixel buffer so it stays valid during sampling
        bufferLock.lock()
